...
Нажимаем SHIFT+I => вводим комментарий (без ковычек и прочего) => ESC => SHIFT+; (не на цифрах) => wq => ENTER
...
Если команда **git status** дает недостаточно подробный, с вашей точки зрения, результат, например если вы хотите не только получить список отредактированных файлов, но и узнать, что именно изменилось, воспользуйтесь командой:

    git diff <=> разница между текущим состоянием файла и состоянием которое сохранено
...
Команда **git status** отвечает на эти вопросы в общей форме, перечисляя имена файлов, а команда **git diff** показывает добавленные и удаленные строки — то есть все вставки в программу.
